...
double LinearProgramSolverGurobi::solve(std::vector<double> &assignments)
{
    ...
    try
    {
        ...
        {
            GRBCallback *gurobiCustomCallback = getGurobiCustomCallback(updateHistory);
            gurobiModel->setCallback(gurobiCustomCallback);
        }
        ...
        if (gurobiModel->get(GRB_IntAttr_Status) == GRB_TIME_LIMIT)
        {
            ...
            if (gurobiModel->get(GRB_IntAttr_SolCount) > 0)
            {
...
void LinearProgramSolverGurobi::setInitialSolution(double *initialSolution)
{
    gurobiModel->set(GRB_DoubleAttr_Start, gurobiModel->getVars(), initialSolution, gurobiVariables.size());
}

GRBCallback *LinearProgramSolverGurobi::getGurobiCustomCallback(UpdateHistory* updateHistory)
{
    if (updateHistory->update_type() == numerical_bounds)
    {
        GRBCallback *gurobiCustomCallback = new GurobiCustomCallbackType0( (LPObjUpdateHistory*) updateHistory);   
        return gurobiCustomCallback;
    } else if (updateHistory->update_type() == assignments) {
        int n_vars = gurobiVariables.size();
        auto xvars = gurobiModel->getVars();
        GRBCallback *gurobiCustomCallback = new GurobiCustomCallbackType1( (LPAssignmentsUpdateHistory*) updateHistory, xvars, n_vars);
        return gurobiCustomCallback; 
    } else {
        std::cout << "This update history is incompatible with any Gurobi Custom Callback available" << std::endl;
        return NULL;
    }
}