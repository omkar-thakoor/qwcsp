...
public:
     ...
     */
    MWVCSolverLinearProgramming(LinearProgramSolver* lps, LPAssignmentsUpdateHistory* updateHistory)
    {
        this->lps.reset(lps);
    ...
    void setInitialSolution(double *initialSolution)
    {
                         ...
                         typename std::map<typename CCG::variable_id_t, bool>& out)
    {
        ...
        for (auto it = vi; it != vi_end; ++ it)
            this->vertex_to_id.insert(std::make_pair(*it, lps->addVariable(vertex_weight_map[*it])));
        ...
        for (auto it = vi; it != vi_end; ++ it)
        {
            ...
            if (id >= 0)
                out[id] = assignments.at(this->vertex_to_id[*it]) > 0.5;
        }
